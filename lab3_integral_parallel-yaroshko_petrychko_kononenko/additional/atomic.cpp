...
void f1(double x_0, double y_0, double x_end, double y_end, double delta_x, double delta_y, ifunc function, std::atomic<double>& sum) {
    ...
    double atomic_sum = sum.load();
    atomic_sum += partial_sum;
    sum.store(atomic_sum);
}
...
void integrate(const ifunc function, const po::variables_map& map, size_t thread_amount) {
    ...
    while (
        ...
        for (size_t i = 0; i < thread_amount; ++i) {
            threads.emplace_back(std::thread(f1, x_0 + delta_x * i, y_0, x_end, y_end, thread_amount * delta_x, delta_y, std::ref(function), std::ref(sum)));
        }
