...
double f1(data_chunk& data, ifunc function) {
    double sum = 0;
    double i = data.x_0;
    double j = data.y_0;
...
void process_data(threadsafe_queue<data_chunk>& input_queue, threadsafe_queue<double>& output_queue, const ifunc function) {
    while (true) {
        data_chunk data = input_queue.dequeue();

        if (data.delta_x == -1) {
            input_queue.enqueue({-1, -1, -1, -1, -1, -1});
            break;
        }

        double result = f1(data, function);
...
void integrate(const ifunc function, const po::variables_map& map, size_t thread_amount, int points_per_thread) {
    ...
    threadsafe_queue<data_chunk> input_queue;
    threadsafe_queue<double> output_queue;

    size_t chunks = std::ceil(steps_x / points_per_thread);

    for (size_t i = 0; i < thread_amount; ++i) {
        threads.emplace_back(process_data, std::ref(input_queue), std::ref(output_queue), std::ref(function));
    }
