...
EXE_FILE = "integrate_parallel_tpool"
...
def record_results(results, threads: int, points_per_thread: int):
    if not os.path.exists(f"data/results/with_thread_pool_{points_per_thread}.json"):
        file = open(f"data/results/with_thread_pool_{points_per_thread}.json", "x")
        file.close()
    try:
        with open(f"data/results/with_thread_pool_{points_per_thread}.json") as file:
            data = json.load(file)
    ...
    with open(f"data/results/with_thread_pool_{points_per_thread}.json", "w") as file:
        json.dump(data, file)


def main():
    n, threads, points_per_thread = parse_args("Parser", "CLA parser for lab5")
